...
from functions.svd_ddprm import ddprm_diffusion, ddprm_plus_diffusion
...
class Diffusion(object):
    ...
    def sample(self, simplified):
        ...
        if simplified:
            print('Run Simplified Priva, without SVD.',
            ...
            self.simplified_ddprm_plus(model, cls_fn)
        else:
            print('Run SVD-based Priva.',
                  f'{self.config.time_travel.T_sampling} sampling steps.',
                  f'travel_length = {self.config.time_travel.travel_length},',
                  f'travel_repeat = {self.config.time_travel.travel_repeat}.',
                  f'Task: {self.args.deg}.'
                 )
            self.svd_based_ddprm_plus(model, cls_fn)
            
            
    def simplified_ddprm_plus(self, model, cls_fn):
    ...
    def svd_based_ddprm_plus(self, model, cls_fn):
        args, config = self.args, self.config
        ...
        for x_orig, classes in pbar:
            ...
            with torch.no_grad():
                if sigma_y==0.: # noise-free case, turn to ddprm
                    x, _ = ddprm_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=classes, config=config)
                else: # noisy case, turn to ddprm+
                    x, _ = ddprm_plus_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, sigma_y, cls_fn=cls_fn, classes=classes, config=config)
